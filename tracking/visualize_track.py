...
def visualize_tracking_data(json_path, video_path=None, confidence_threshold=0.1):
    ...
    def update_plot():
        ...
        ax.text(
            5,
            10,  # Offset text position slightly
        ...
        for obj in frame_info["objects"]:
            ...
            color = class_colors.get(
                class_id, "white"
            )  # Default to white if class is unknown
            ax.scatter(
                center[0],
                center[1],
            ...
            ax.text(
                center[0] - 7,
                center[1] - 5,  # Offset text position slightly
                f"ID: {track_id}",
                color="white",
                fontsize=8,
                bbox=dict(facecolor="black", alpha=0.7, edgecolor="none", pad=1),
        ...
        if frame is not None:
            ...
            ax.imshow(
                frame_rgb, alpha=0.9, extent=[0, frame.shape[1], frame.shape[0], 0]
            )
        ...
        handles = [
            ...
            if any(obj["class_id"] == cls for obj in frame_info["objects"])
        ]
        ax.legend(
    ...
    def next_frame(event=None):
        ...
        current_frame["index"] = (current_frame["index"] + 1) % (
            min(total_frames, len(tracking_data))
        )
        update_plot()
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or len(sys.argv) > 4:
        print(
            f"Usage: python {sys.argv[0]} <jsonname> <videoname> [confidence_threshold]"
        )
        sys.exit(1)
